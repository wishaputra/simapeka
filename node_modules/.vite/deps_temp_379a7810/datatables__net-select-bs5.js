...
// node_modules/datatables.net-select-bs5/node_modules/datatables.net/js/jquery.dataTables.mjs
...
// node_modules/datatables.net-select/node_modules/datatables.net/js/jquery.dataTables.mjs
...
jquery_dataTables_default2.select.version = "1.7.0-dev";
jquery_dataTables_default2.select.init = function(dt) {
  ...
  $4.fn.dataTable.ext.order["select-checkbox"] = function(settings, col) {
    return this.api().column(col, { order: "index" }).nodes().map(function(td) {
      if (settings._select.items === "row") {
        return $4(td).parent().hasClass(settings._select.className);
      } else if (settings._select.items === "cell") {
        return $4(td).hasClass(settings._select.className);
      }
      return false;
    });
  };
  if (!setStyle && $4(dt.table().node()).hasClass("selectable")) {
    dt.select.style("os");
  }
};
...
function disableMouseSelection(dt) {
  ...
  $4(dt.table().container()).off("mousedown.dtSelect", selector).off("mouseup.dtSelect", selector).off("click.dtSelect", selector);
  $4("body").off("click.dtSelect" + _safeId(dt.table().node()));
}
function enableMouseSelection(dt) {
  var container = $4(dt.table().container());
  var ctx = dt.settings()[0];
  var selector = ctx._select.selector;
  var matchSelection;
  container.on("mousedown.dtSelect", selector, function(e) {
    ...
    if (matchSelection) {
      ...
      if (!selection.anchorNode || $4(selection.anchorNode).closest("table")[0] === dt.table().node()) {
    ...
    if ($4(e.target).closest("div." + wrapperClass)[0] != dt.table().container()) {
      return;
    }
    var cell = dt.cell($4(e.target).closest("td, th"));
    if (!cell.any()) {
      return;
    }
    var event = $4.Event("user-select.dt");
  ...
  $4("body").on("click.dtSelect" + _safeId(dt.table().node()), function(e) {
    if (ctx._select.blurable) {
      if ($4(e.target).parents().filter(dt.table().container()).length) {
        return;
      }
      if ($4(e.target).parents("html").length === 0) {
        return;
      }
      if ($4(e.target).parents("div.DTE").length) {
        return;
      }
      var event = $4.Event("select-blur.dt");
...
function eventTrigger(api, type, args, any) {
  ...
  args.unshift(api);
  $4(api.table().node()).trigger(type, args);
}
function info(api) {
  ...
  var add = function(el, name, num) {
    el.append(
      $4('<span class="select-item"/>').append(
        api.i18n(
          "select." + name + "s",
  ...
  $4.each(ctx.aanFeatures.i, function(i, el) {
    el = $4(el);
    var output = $4('<span class="select-info"/>');
    add(output, "row", rows);
...
function init(ctx) {
  ...
  ctx.aoRowCreatedCallback.push({
    fn: function(row, data, index) {
      ...
      if (d._select_selected) {
        $4(row).addClass(ctx._select.className);
      }
      for (i = 0, ien = ctx.aoColumns.length; i < ien; i++) {
        if (ctx.aoColumns[i]._select_selected || d._selected_cells && d._selected_cells[i]) {
          $4(d.anCells[i]).addClass(ctx._select.className);
        }
      }
    },
  ...
  api.on("destroy.dtSelect", function() {
    $4(api.rows({ selected: true }).nodes()).removeClass(api.settings()[0]._select.className);
    disableMouseSelection(api);
    api.off(".dtSelect");
    $4("body").off(".dtSelect" + _safeId(api.table().node()));
  });
}
function rowColumnRange(dt, type, idx, last) {
  var indexes = dt[type + "s"]({ search: "applied" }).indexes();
  var idx1 = $4.inArray(last, indexes);
  var idx2 = $4.inArray(idx, indexes);
  if (!dt[type + "s"]({ selected: true }).any() && idx1 === -1) {
    indexes.splice($4.inArray(idx, indexes) + 1, indexes.length);
  } else {
    if (idx1 > idx2) {
      var tmp = idx2;
      idx2 = idx1;
  ...
  if (!dt[type](idx, { selected: true }).any()) {
    ...
    indexes.splice($4.inArray(idx, indexes), 1);
...
apiRegisterPlural("rows().select()", "row().select()", function(select) {
  ...
  this.iterator("row", function(ctx, idx) {
    ...
    $4(ctx.aoData[idx].nTr).addClass(ctx._select.className);
  });
...
apiRegisterPlural("columns().select()", "column().select()", function(select) {
  ...
  this.iterator("column", function(ctx, idx) {
    ...
    $4(column.header()).addClass(ctx._select.className);
    $4(column.footer()).addClass(ctx._select.className);
...
apiRegisterPlural("cells().select()", "cell().select()", function(select) {
  ...
  this.iterator("cell", function(ctx, rowIdx, colIdx) {
    ...
    if (data.anCells) {
      $4(data.anCells[colIdx]).addClass(ctx._select.className);
    }
  });
  this.iterator("table", function(ctx, i) {
...
apiRegisterPlural("rows().deselect()", "row().deselect()", function() {
  ...
  this.iterator("row", function(ctx, idx) {
    ...
    $4(ctx.aoData[idx].nTr).removeClass(ctx._select.className);
  });
  this.iterator("table", function(ctx, i) {
...
apiRegisterPlural("columns().deselect()", "column().deselect()", function() {
  ...
  this.iterator("column", function(ctx, idx) {
    ...
    $4(column.header()).removeClass(ctx._select.className);
    $4(column.footer()).removeClass(ctx._select.className);
    api2.cells(null, idx).indexes().each(function(cellIdx) {
      var data = ctx.aoData[cellIdx.row];
      var cellSelected = data._selected_cells;
      if (data.anCells && (!cellSelected || !cellSelected[cellIdx.column])) {
        $4(data.anCells[cellIdx.column]).removeClass(ctx._select.className);
      }
    });
  });
  this.iterator("table", function(ctx, i) {
...
apiRegisterPlural("cells().deselect()", "cell().deselect()", function() {
  ...
  this.iterator("cell", function(ctx, rowIdx, colIdx) {
    ...
    if (data.anCells && !ctx.aoColumns[colIdx]._select_selected) {
      $4(data.anCells[colIdx]).removeClass(ctx._select.className);
    }
  });
  this.iterator("table", function(ctx, i) {
...
function enabled(dt, config) {
  if ($4.inArray("rows", config.limitTo) !== -1 && dt.rows({ selected: true }).any()) {
    return true;
  }
  if ($4.inArray("columns", config.limitTo) !== -1 && dt.columns({ selected: true }).any()) {
    return true;
  }
  if ($4.inArray("cells", config.limitTo) !== -1 && dt.cells({ selected: true }).any()) {
    return true;
  }
  return false;
}
var _buttonNamespace = 0;
$4.extend(jquery_dataTables_default2.ext.buttons, {
  selected: {
...
$4.each(["Row", "Column", "Cell"], function(i, item) {
...
$4.fn.DataTable.select = jquery_dataTables_default2.select;
$4(document).on("preInit.dt.dtSelect", function(e, ctx) {
  if (e.namespace !== "dt") {
    return;
  }
  jquery_dataTables_default2.select.init(new jquery_dataTables_default2.Api(ctx));
