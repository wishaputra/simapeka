...
// node_modules/datatables.net-fixedheader-bs5/node_modules/datatables.net/js/jquery.dataTables.mjs
...
// node_modules/datatables.net-fixedheader/node_modules/datatables.net/js/jquery.dataTables.mjs
...
var FixedHeader = function(dt, config) {
  ...
  this.c = $4.extend(true, {}, FixedHeader.defaults, config);
  this.s = {
    dt,
    position: {
      theadTop: 0,
  ...
  this.dom = {
    floatingHeader: null,
    thead: $4(dt.table().header()),
    tbody: $4(dt.table().body()),
    tfoot: $4(dt.table().footer()),
    header: {
      host: null,
      floating: null,
      floatingParent: $4('<div class="dtfh-floatingparent">'),
      placeholder: null
    },
    footer: {
      host: null,
...
$4.extend(FixedHeader.prototype, {
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
   * API methods
   */
  /**
  ...
  update: function(force) {
    ...
    if ($4(table).children("thead").length === 0) {
      return;
    }
    this._positions();
  ...
  _constructor: function() {
    var that = this;
    var dt = this.s.dt;
    $4(window).on("scroll" + this.s.namespace, function() {
      that._scroll();
    }).on(
      "resize" + this.s.namespace,
      jquery_dataTables_default2.util.throttle(function() {
        that.s.position.windowHeight = $4(window).height();
        that.update();
      }, 50)
    );
    var autoHeader = $4(".fh-fixedHeader");
    if (!this.c.headerOffset && autoHeader.length) {
      this.c.headerOffset = autoHeader.outerHeight();
    }
    var autoFooter = $4(".fh-fixedFooter");
  ...
  _clone: function(item, force) {
    ...
    if (!force && itemDom.floating) {
      ...
      var tableNode = $4(dt.table().node());
      var scrollBody = $4(tableNode.parent());
      var scrollEnabled = this._scrollEnabled();
      itemDom.floating = $4(dt.table().node().cloneNode(false)).attr("aria-hidden", "true").css({
        "table-layout": "fixed",
        top: 0,
        left: 0
  ...
  _stickyPosition: function(el, sign) {
    if (this._scrollEnabled()) {
      var that = this;
      var rtl = $4(that.s.dt.table().node()).css("direction") === "rtl";
      el.find("th").each(function() {
        if ($4(this).css("position") === "sticky") {
          var right = $4(this).css("right");
          var left = $4(this).css("left");
          if (right !== "auto" && !rtl) {
            var potential = +right.replace(/px/g, "") + (sign === "-" ? -1 : 1) * that.s.dt.settings()[0].oBrowser.barWidth;
            $4(this).css("right", potential > 0 ? potential : 0);
          } else if (left !== "auto" && rtl) {
            var potential = +left.replace(/px/g, "") + (sign === "-" ? -1 : 1) * that.s.dt.settings()[0].oBrowser.barWidth;
            $4(this).css("left", potential > 0 ? potential : 0);
          }
        }
      });
    }
  ...
  _matchWidths: function(from, to) {
    var get = function(name) {
      return $4(name, from).map(function() {
        return $4(this).css("width").replace(/[^\d\.]/g, "") * 1;
      }).toArray();
    };
    var set = function(name, toWidths) {
      $4(name, to).each(function(i) {
        $4(this).css({
          width: toWidths[i],
          minWidth: toWidths[i]
        });
  ...
  _horizontal: function(item, scrollLeft) {
    ...
    if (itemDom.floating && lastScrollLeft[item] !== scrollLeft) {
      if (this._scrollEnabled()) {
        var newScrollLeft = $4($4(this.s.dt.table().node()).parent()).scrollLeft();
  ...
  _modeChange: function(mode, item, forceChange) {
    ...
    var focus = $4.contains(tablePart[0], document.activeElement) ? document.activeElement : null;
    var scrollBody = $4($4(this.s.dt.table().node()).parent());
    if (mode === "in-place") {
      ...
      $4($4(itemDom.host.parent()).parent()).scrollLeft(scrollBody.scrollLeft());
    } else if (mode === "in") {
      this._clone(item, forceChange);
      var scrollOffset = scrollBody.offset();
      var windowTop = $4(document).scrollTop();
      var windowHeight = $4(window).height();
  ...
  _positions: function() {
    ...
    var position = this.s.position;
    var dom = this.dom;
    var tableNode = $4(table.node());
    var scrollEnabled = this._scrollEnabled();
    var thead = $4(dt.table().header());
    var tfoot = $4(dt.table().footer());
    var tbody = dom.tbody;
  ...
  _scroll: function(forceChange) {
    ...
    var scrollEnabled = this._scrollEnabled();
    var scrollBody = $4(this.s.dt.table().node()).parent();
    var scrollOffset = scrollBody.offset();
    var scrollHeight = scrollBody.outerHeight();
    var windowLeft = $4(document).scrollLeft();
    var windowTop = $4(document).scrollTop();
    var windowHeight = $4(window).height();
    ...
    if (this.c.footer && this.dom.tfoot.length) {
      ...
      if (scrollEnabled && footer.offset.top > windowTop) {
        ...
        if (Math.round(scrollBody.outerHeight()) >= Math.round(newHeight)) {
          $4(this.dom.tfoot.parent()).addClass("fixedHeader-floating");
        } else {
          $4(this.dom.tfoot.parent()).removeClass("fixedHeader-floating");
        }
      }
    }
    ...
    if (this.s.dt.settings()[0]._fixedColumns !== void 0) {
      var adjustBlocker = function(side, end, el) {
        if (el === void 0) {
          var blocker = $4("div.dtfc-" + side + "-" + end + "-blocker");
...
$4.fn.dataTable.FixedHeader = FixedHeader;
$4.fn.DataTable.FixedHeader = FixedHeader;
$4(document).on("init.dt.dtfh", function(e, settings, json) {
  if (e.namespace !== "dt") {
    return;
  }
  var init = settings.oInit.fixedHeader;
  var defaults = jquery_dataTables_default2.defaults.fixedHeader;
...
jquery_dataTables_default2.Api.register("fixedHeader()", function() {
});
jquery_dataTables_default2.Api.register("fixedHeader.adjust()", function() {
  return this.iterator("table", function(ctx) {
...
jquery_dataTables_default2.Api.register("fixedHeader.enable()", function(flag) {
  return this.iterator("table", function(ctx) {
    var fh = ctx._fixedHeader;
...
jquery_dataTables_default2.Api.register("fixedHeader.enabled()", function() {
...
jquery_dataTables_default2.Api.register("fixedHeader.disable()", function() {
  return this.iterator("table", function(ctx) {
    var fh = ctx._fixedHeader;
    if (fh && fh.enabled()) {
      fh.enable(false);
...
$4.each(["header", "footer"], function(i, el) {
  jquery_dataTables_default2.Api.register("fixedHeader." + el + "Offset()", function(offset) {
    var ctx = this.context;
    if (offset === void 0) {
