...
// node_modules/jquery-datatables-checkboxes/node_modules/datatables.net/js/jquery.dataTables.mjs
  ...
  "node_modules/jquery-datatables-checkboxes/js/dataTables.checkboxes.js"(exports, module) {
    (function(factory) {
      ...
      var Checkboxes = function(settings) {
        if (!DataTable2.versionCheck || !DataTable2.versionCheck("1.10.8")) {
        ...
        */
        _constructor: function() {
          ...
          for (var i = 0; i < ctx.aoColumns.length; i++) {
            if (ctx.aoColumns[i].checkboxes) {
              var $colHeader = $2(dt.column(i).header());
              hasCheckboxes = true;
              if (!$2.isPlainObject(ctx.aoColumns[i].checkboxes)) {
                ctx.aoColumns[i].checkboxes = {};
              }
              ctx.aoColumns[i].checkboxes = $2.extend(
                {},
                Checkboxes.defaults,
                ctx.aoColumns[i].checkboxes
              );
              ...
              DataTable2.ext.internal._fnColumnOptions(ctx, i, colOptions);
              $colHeader.removeClass("sorting");
              $colHeader.off(".dt");
              if (ctx.sAjaxSource === null) {
                var cells = dt.cells("tr", i);
                cells.invalidate("data");
                $2(cells.nodes()).addClass(colOptions["className"]);
              }
              self.s.data[i] = {};
              ...
              if (ctx.aoColumns[i].checkboxes.selectAll) {
                ...
                if (ctx.aoColumns[i].checkboxes.selectAllRender !== null) {
                  var selectAllHtml = "";
                  if ($2.isFunction(ctx.aoColumns[i].checkboxes.selectAllRender)) {
          ...
          if (hasCheckboxes) {
            self.loadState();
            var $table = $2(dt.table().node());
            var $tableBody = $2(dt.table().body());
            var $tableContainer = $2(dt.table().container());
            ...
            $tableContainer.on("click.dtCheckboxes", "thead th.dt-checkboxes-select-all", function() {
              $2('input[type="checkbox"]', this).not(":disabled").trigger("click");
            });
            if (!hasCheckboxesSelectRow) {
            ...
            $2(document2).on("click.dtCheckboxes", '.fixedHeader-floating thead th.dt-checkboxes-select-all input[type="checkbox"]', function(e) {
              if (ctx._fixedHeader) {
            ...
            $2(document2).on("click.dtCheckboxes", ".fixedHeader-floating thead th.dt-checkboxes-select-all", function() {
              if (ctx._fixedHeader) {
                if (ctx._fixedHeader.dom["header"].floating) {
                  $2('input[type="checkbox"]', this).trigger("click");
                }
              }
            });
        ...
        onDataTablesInit: function() {
          ...
          if (!ctx.oFeatures.bServerSide) {
            ...
            $2(dt.table().node()).on("xhr.dt.dtCheckboxes", function(e, settings, json, xhr) {
        ...
        onDataTablesStateSave: function(e, settings, data) {
          var self = this;
          var ctx = self.s.ctx;
          $2.each(self.s.columns, function(index, colIdx) {
        ...
        onDataTablesDestroy: function() {
          ...
          var $tableBody = $2(dt.table().body());
          var $tableContainer = $2(dt.table().container());
          $2(document2).off("click.dtCheckboxes");
          $tableContainer.off(".dtCheckboxes");
          $tableBody.off(".dtCheckboxes");
          $table.off(".dtCheckboxes");
          self.s.data = {};
          self.s.dataDisabled = {};
          $2(".dt-checkboxes-select-all", $table).each(function(index, el) {
            $2(el).html($2(el).data("html")).removeClass("dt-checkboxes-select-all");
          });
        },
        ...
        updateCheckbox: function(cells, colIdx, isSelected) {
          ...
          if (cellNodes.length) {
            $2("input.dt-checkboxes", cellNodes).not(":disabled").prop("checked", isSelected);
            if ($2.isFunction(ctx.aoColumns[colIdx].checkboxes.selectCallback)) {
        ...
        updateStateCheckboxes: function(opts) {
          ...
          dt.cells("tr", self.s.columns, opts).every(function(rowIdx, colIdx) {
            ...
            if (!isCellSelectable) {
              $2("input.dt-checkboxes", this.node()).prop("disabled", true);
            }
          });
        },
        ...
        updateSelectAll: function(colIdx) {
          ...
          if (ctx.aoColumns[colIdx].checkboxes && ctx.aoColumns[colIdx].checkboxes.selectAll) {
            ...
            var $checkboxesSelectAll = $2('.dt-checkboxes-select-all[data-col="' + colIdx + '"] input[type="checkbox"]', $tableContainer);
            var countChecked = 0;
            var countDisabled = 0;
            var cellsData = cells.data();
            $2.each(cellsData, function(index, cellData) {
              if (self.isCellSelectable(colIdx, cellData)) {
            ...
            if (ctx._fixedHeader) {
              if (ctx._fixedHeader.dom["header"].floating) {
                $checkboxesSelectAll = $2('.fixedHeader-floating .dt-checkboxes-select-all[data-col="' + colIdx + '"] input[type="checkbox"]');
              }
            }
            var isSelected;
            var isIndeterminate;
            ...
            if (isChanged || isSelectedNow !== isSelected || isIndeterminateNow !== isIndeterminate) {
              ...
              if ($2.isFunction(ctx.aoColumns[colIdx].checkboxes.selectAllCallback)) {
        ...
        showInfoSelected: function() {
          ...
          if (colIdx !== null) {
            ...
            var add = function($el, name, num) {
              $el.append($2('<span class="select-item"/>').append(dt.i18n(
                "select." + name + "s",
                { _: "%d " + name + "s selected", 0: "", 1: "1 " + name + " selected" },
                num
              )));
            };
            $2.each(ctx.aanFeatures.i, function(i, el) {
              var $el = $2(el);
        ...
        updateFixedColumn: function(colIdx) {
          ...
          if (ctx._oFixedColumns) {
            ...
            if (colIdx < leftCols || colIdx > rightCols) {
              ...
              setTimeout(function() {
                $2.each(self.s.columns, function(index, colIdx2) {
      ...
      Api.registerPlural("columns().checkboxes.select()", "column().checkboxes.select()", function(state) {
        ...
        return this.iterator("column-rows", function(ctx, colIdx, i, j, rowsIdx) {
          if (ctx.aoColumns[colIdx].checkboxes) {
            var selector = [];
            $2.each(rowsIdx, function(index, rowIdx) {
      ...
      Api.registerPlural("cells().checkboxes.enable()", "cell().checkboxes.enable()", function(state) {
        ...
        return this.iterator("cell", function(ctx, rowIdx, colIdx) {
          if (ctx.aoColumns[colIdx].checkboxes) {
            ...
            if (cellNode) {
              $2("input.dt-checkboxes", cellNode).prop("disabled", !state);
            }
            if (ctx.aoColumns[colIdx].checkboxes.selectRow) {
              if (Object.prototype.hasOwnProperty.call(ctx.checkboxes.s.data, colIdx) && Object.prototype.hasOwnProperty.call(ctx.checkboxes.s.data[colIdx], cellData)) {
                ctx.checkboxes.updateSelect(rowIdx, state);
      ...
      Api.registerPlural("columns().checkboxes.selected()", "column().checkboxes.selected()", function() {
        return this.iterator("column-rows", function(ctx, colIdx, i, j, rowsIdx) {
          if (ctx.aoColumns[colIdx].checkboxes) {
            var data = [];
            if (ctx.oFeatures.bServerSide) {
              $2.each(ctx.checkboxes.s.data[colIdx], function(cellData) {
                if (ctx.checkboxes.isCellSelectable(colIdx, cellData)) {
                  data.push(cellData);
                }
              });
      ...
      $2.fn.DataTable.Checkboxes = Checkboxes;
      $2.fn.dataTable.Checkboxes = Checkboxes;
      $2(document2).on("preInit.dt.dtCheckboxes", function(e, settings) {
        if (e.namespace !== "dt") {
          return;
        }
